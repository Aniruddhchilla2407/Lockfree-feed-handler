...
typedef struct {
    ...
    volatile int *start_flag;
    uint64_t      received;
    uint64_t     *latencies_ns;   // one entry per tick received
} ConsCtx;
...
static void *producer(void *arg) {
    ...
    for (int i = 0; i < ctx->count; i++) {
        t.price = (double)i;
        t.timestamp_ns = stats_now_ns();   // record send time
        while (!ring_buffer_push(ctx->rb, &t)) { /* retry */ }
    }
    *(ctx->stop_flag) = 1;
    return NULL;
}
...
static void *consumer(void *arg) {
    ...
    for (;;) {
        if (ring_buffer_pop(ctx->rb, &t)) {
            ...
            while (ring_buffer_pop(ctx->rb, &t)) {
                uint64_t now = stats_now_ns();
                ctx->latencies_ns[received] = now - t.timestamp_ns;
                received++;
            }
...
int main(void) {
    ...
    fclose(f);
    printf("Results written to bench/results/lockfree.csv\n");
    return 0;
}